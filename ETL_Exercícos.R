...
#Arquivo com Plano de Interoperabilidade
#Vantagem:
#Desvantagem:
...
# carrega base de dados em formato tabular(.xlsx) - padrão para interoperabilidade
sinistrosRecife <- read_xlsx('bases_tratadas/sinistrosRecife.xlsx')

# compara os três processos de exportação, usando um microbenchmark de função

microbenchmark(a <- saveRDS(sinistrosRecifeRaw, "bases_tratadas/sinistrosRecife.rds"), b <- write.csv2(sinistrosRecifeRaw, "bases_tratadas/sinistrosRecife.csv"), c <- write_xlsx(sinistrosRecifeRaw, "bases_tratadas/sinistrosRecife.xlsx"), times = 10L)

microbenchmark(a <- readRDS('bases_tratadas/sinistrosRecife.rds'), b <- read.csv2('bases_tratadas/sinistrosRecife.csv', sep = ';'), c <- read_xlsx('bases_tratadas/sinistrosRecife.xlsx'), times = 10L)

# Utilizando a biblioteca microbenchmark para avaliar a otimização dos formatos foi observado que na função exportar  o saveRDS foi mais eficaz como também na função readRDS.
